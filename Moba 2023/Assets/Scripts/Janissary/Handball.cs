...
public class Handball : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {   
            if (!attacker.parent.GetComponent<Statsinfo>().isAlive || (attacker.parent.GetComponent<Statsinfo>().target.parent != null && !attacker.parent.GetComponent<Statsinfo>().target.parent.GetComponent<Statsinfo>().isAlive))
            {
                Destroy(gameObject);
            }
        }
